resource "aws_rds_cluster" "rds-cluster" {
  cluster_identifier              = lower("aws-rds-cluster-dev-jasonseo-aurora-jasonseo-db")
...
resource "aws_rds_cluster_instance" "rds-instance" {
  ...
  identifier                  = lower("aws-rds-instance-dev-jasonseo-aurora-jasonseodb-${count.index}")
...
resource "random_password" "rds_password" {
  length           = 8
  special          = true
  override_special = "!" #비밀번호에서 ! 문자만 허용한다는 설정(다른 특수문자는 제외).
  #override_special = "/_%@ "
}

# --- !!! Secrets Manager 리소스 추가 !!! ---
resource "aws_secretsmanager_secret" "aurora_db_password_secret" {
  # 이름은 루트에서 전달된 변수들을 사용하여 구성
  name        = "${var.stage}/${var.servicename}/${var.dbname}-aurora-master-password"
  description = "Master password for the Aurora DB cluster for ${var.servicename}-${var.dbname} in ${var.stage}"
  
  recovery_window_in_days = 0   # 0 ⇒ 즉시 삭제옵션 (dev 환경에서만!)
  
  tags = var.tags # 루트에서 merge된 태그가 전달됨
}

resource "aws_secretsmanager_secret_version" "aurora_db_password_version" {
  secret_id     = aws_secretsmanager_secret.aurora_db_password_secret.id
  secret_string = random_password.rds_password.result # 생성된 랜덤 비밀번호 사용

  # Secret 값이 Terraform 외부에서 변경될 수 있다면 ignore_changes 추가 고려
  lifecycle {
    ignore_changes = [secret_string]
  }
}
# --- !!! Secrets Manager 리소스 추가 끝 !!! ---
...
resource "aws_rds_cluster_parameter_group" "rds-cluster-parameter-group" {
  name=lower("aws-rds-cluster-parameter-group-dev-jasonseo-aurora-jasonseo-db")
...
resource "aws_db_parameter_group" "rds-instance-parameter-group" {
  name        = lower("aws-rds-instance-parameter-group-dev-jasonseo-aurora-jasonseo-db")
