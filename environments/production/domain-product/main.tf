terraform {
  ...
  backend "s3" {
    bucket         = "jasonseo-production-terraform-state"
    key            = "terraform/production/domain-product/terraform.tfstate"
    region         = "ca-central-1"
    dynamodb_table = "jasonseo-production-terraform-lock"
    use_lockfile   = true
    encrypt        = true
  }
}
...
data "terraform_remote_state" "core_infra" {
  ...
  config = {
    bucket = "jasonseo-production-terraform-state"
    key    = "terraform/production/core-infra/terraform.tfstate"
    region = "ca-central-1"
  }
}
